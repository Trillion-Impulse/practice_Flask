...
# Message Flashing
- 사용자 경험(UX)에서 피드백이 중요
- Flask에서는 사용자에게 피드백을 주는 간단한 방법인 메시지 플래싱(message flashing) 기능을 제공
    - 예를 들어, 로그인 실패나 회원가입 성공 같은 메시지를 사용자에게 일시적으로 보여줄 때 사용 가능
- flash() 함수는 현재 요청이 끝날 때 메시지를 저장
    - 그 다음 요청(예: 로그인 후 리디렉션된 페이지)에서 그 메시지를 불러올 수 있음
- 플래싱 메시지는 보통 HTML 템플릿 파일에 get_flashed_messages()를 사용해 출력 됨
    - 레이아웃 템플릿은 여러 페이지에서 공통으로 쓰이는 HTML 구조인데, 
        이 안에 플래시 메시지를 넣으면 모든 페이지에서 일관되게 메시지를 보여줄 수 있음
- 메시지를 플래시하려면 flash() 메서드를 사용
    - `flash("로그인에 성공했습니다")`와 같이 사용자가 수행한 작업에 대한 메시지를 설정 가능
- 메시지를 얻으려면 get_flashed_messages()를 사용
    - `get_flashed_messages()`를 HTML 템플릿에서 사용해 그 메시지를 화면에 출력 가능
- 전체 사용법은 Flask 공식 문서의 Message Flashing 섹션에서 예제를 통해 확인 가능
...
# Blueprint
- 블루프린트는 관련된 뷰들과 기타 코드들을 그룹화하여 정리하는 방법
- 예를 들어 로그인, 로그아웃, 회원가입 등 인증 관련 기능들을 하나의 블루프린트로 묶어서 모듈화 가능
- Blueprint를 사용하면 코드가 더 깔끔하고 유지보수가 쉬워 짐
- 일반적으로는 @app.route()처럼 직접 Flask 애플리케이션에 경로를 등록하지만, 
    블루프린트를 사용하면 @bp.route() 형식으로 등록하고, 이후 전체 블루프린트를 앱에 한 번에 연결
- Flask에서 흔히 사용되는 애플리케이션 팩토리 패턴(create_app)을 사용할 경우, 
    이 함수 내부에서 블루프린트를 앱에 등록해야 함

## Blueprint vs package
- Blueprint는 Flask 앱에서 사용되는 기능 단위의 모듈
- 패키지는 단순히 Python 파일들을 묶는 폴더 구조
- Blueprint는 패키지 그 자체가 아님
- 패키지 안에 Blueprint를 정의하여 모듈화하는 것이 일반적
- Blueprint는 패키지처럼 보이지만, Flask 애플리케이션의 기능 단위로 작동하는 구성 요소
